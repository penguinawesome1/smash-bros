class Attack extends Component {
    ...
    switch (this.type) {
      case "bullet": {
        this.position = {
          x:
            position.x +
            this.scale * (0 + 50 * (this.direction === "right" ? 1 : 0)),
      ...
      case "potion": {
        this.position = {
          x:
            position.x +
            this.scale * (50 + 50 * (this.direction === "right" ? 1 : -1)),
      ...
      case "boomerang": {
        this.position = {
          x:
            position.x +
            this.scale * (0 + 40 * (this.direction === "right" ? 1 : 0)),
  ...
  update() {
    ...
    if (this.type === "boomerang") {
      this.velocity.x += this.direction === "right" ? -0.2 : 0.2;
  ...
  reactToCollision() {
    ...
    if (this.type !== "homingdart" && this.isCollision()) {
    ...
    if (this.type === "homingdart") {
      if (
        ...
        this.player.velocity.x += Math.cos(angle) * 0.5;
        this.player.velocity.y += Math.sin(angle) * 0.2;
      } else {
        const angle = calcAngle({
          object1: this.hitbox,
          object2: this.otherPlayer.hitbox,
        });
        this.otherPlayer.velocity.x += Math.cos(angle) * 0.5;
        this.otherPlayer.velocity.y += Math.sin(angle) * 0.2;
      }
      return;
    }
    ...
    this.otherPlayer.velocity.x +=
      (Math.cos(angle) * 500) / this.otherPlayer.healthBar.value;
    this.otherPlayer.velocity.y +=
      (Math.sin(angle) * 250) / this.otherPlayer.healthBar.value;
  ...
  updateHitbox() {
    this.hitbox = {
      position: {
        x:
          this.position.x +
          this.scale * (5 + 5 * (this.direction === "right" ? 1 : -1)),
